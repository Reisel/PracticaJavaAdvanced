...
public class Chat extends javax.swing.JFrame {
    public Chat() {
        ...
        //ChatServerRunnable
        new Thread(new ChatServerRunnable(txaChat)).start();
    ...
    private void btnEnviarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEnviarActionPerformed
        // Evento btnEnviar
        try (OutputStream out=new Socket(
                    MapaDirecciones
                            .getMapa()
                            .get(cmbLista.getItemAt(cmbLista.getSelectedIndex()))
                    ,8000).getOutputStream()){ 
            out.write(txtMensaje.getText().getBytes());
        } catch (NoRouteToHostException e){ 
            txaChat.append("No se encontro a "+cmbLista.getItemAt(cmbLista.getSelectedIndex())+"\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
